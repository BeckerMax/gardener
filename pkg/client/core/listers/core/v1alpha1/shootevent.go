...
type ShootEventLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ShootEvent, err error)
...
// List lists all ShootEvents in the indexer.
func (s *shootEventLister) List(selector labels.Selector) (ret []*v1alpha1.ShootEvent, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ShootEvent))
	})
	return ret, err
}
...
type ShootEventNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.ShootEvent, error)
...
// List lists all ShootEvents in the indexer for a given namespace.
func (s shootEventNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ShootEvent, err error) {
...
// Get retrieves the ShootEvent from the indexer for a given namespace and name.
func (s shootEventNamespaceLister) Get(name string) (*v1alpha1.ShootEvent, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("shootevent"), name)
	}
	return obj.(*v1alpha1.ShootEvent), nil
}
