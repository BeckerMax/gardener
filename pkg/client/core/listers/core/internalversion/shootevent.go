...
type ShootEventLister interface {
	...
	List(selector labels.Selector) (ret []*core.ShootEvent, err error)
...
// List lists all ShootEvents in the indexer.
func (s *shootEventLister) List(selector labels.Selector) (ret []*core.ShootEvent, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*core.ShootEvent))
	})
	return ret, err
}
...
type ShootEventNamespaceLister interface {
	...
	Get(name string) (*core.ShootEvent, error)
...
// List lists all ShootEvents in the indexer for a given namespace.
func (s shootEventNamespaceLister) List(selector labels.Selector) (ret []*core.ShootEvent, err error) {
...
// Get retrieves the ShootEvent from the indexer for a given namespace and name.
func (s shootEventNamespaceLister) Get(name string) (*core.ShootEvent, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(core.Resource("shootevent"), name)
	}
	return obj.(*core.ShootEvent), nil
}
